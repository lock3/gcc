...
struct S
{
  int now(int a, T *t) [[ pre: a > 0 ]] [[ pre: x < 0 ]];// [[ pre: t->pri > 0 ]];
  int later(int a, T *t);
  int both(int a, T *t) [[ pre: a > 0 ]] [[ pre: x < 0 ]];// [[ pre: t->pri > 0 ]];
  int x{-1};
};

int now(int a, T *t) [[ pre: a > 0 ]];// [[ pre: t->pri > 0 ]];
int later(int a, T *t);
int both(int a, T *t) [[ pre: a > 0 ]];// [[ pre: t->pri > 0 ]];

struct T
{
  friend int now(int a, T *t);
  friend int later(int a, T *t) [[ pre: a > 0 ]];// [[ pre: t->pri > 0 ]];
  friend int both(int a, T *t) [[ pre: a > 0 ]];// [[ pre: t->pri > 0 ]];

  friend int S::now(int a, T *t);
  friend int S::later(int a, T *t) [[ pre: a > 0 ]] [[ pre: x < 0 ]];// [[ pre: t->pri > 0 ]];
  friend int S::both(int a, T *t) [[ pre: a > 0 ]] [[ pre: x < 0 ]];// [[ pre: t->pri > 0 ]];

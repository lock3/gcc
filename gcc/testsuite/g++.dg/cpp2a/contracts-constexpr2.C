...
// { dg-do run }
// { dg-options "-std=c++20 -Kpre=observe -Kpost=observe" }
...
int main(int, char **) {
  ...
  // { dg-error "contract predicate" "" { target *-*-* } 8 }
  constexpr int f = ftfun(5);
  // { dg-error "contract predicate" "" { target *-*-* } 16 }
  constexpr int g = explicitfn(5);
  // { dg-error "contract predicate" "" { target *-*-* } 25 }
  constexpr int h = ftfun2(5.5);
  // { dg-error "contract predicate" "" { target *-*-* } 33 }

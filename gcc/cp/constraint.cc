  ...
  if (decl_p)
  ...
  else
    {
      ...
      for (hash_table<sat_hasher>::iterator i = sat_cache->begin (); i != end;
        ...
        {
          sat_entry *e = *i;
          if (!cb (decl_p, e->atom, e->args, e->result, ctx))
            return;
        }
    }
}
                   ...
                   tree result)
{
  if (decl_p)
    ...
    {
      // TODO FIXME: this changed significantly; merge into satisfaction_cache::save?
      //save_satisfaction (decl_or_constraint, args, result);
    }
  if (!args) return; // TODO FIXME remove; suppresses unused arg warning
}
